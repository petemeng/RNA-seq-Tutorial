...
**图 2：GC 含量分布。** 干旱处理组与对照组的 GC 含量分布几乎完全重叠，说明建库和测序没有引入明显的 GC 偏好。
...
**图 3：接头含量分布。** 原始数据中可检测到的接头含量极低，经 fastp 处理后已完全去除。
...
数据洗干净了。下一篇我们正式进入定量（quantification）环节：把 clean FASTQ 变成"每个基因在每个样本里有多活跃"的表达矩阵。

我们会回答这些问题：Salmon 为什么能在不做传统全长比对的情况下快速完成定量？transcript-level 和 gene-level 结果是什么关系？什么时候还需要保留 BAM 和 featureCounts 这条传统路线？以及——如果不同方法算出来的表达量不完全一致，你应该优先信什么？
